...
async def generate_pdf_report(session_id: str, db: DBSession = Depends(get_db)):
    try:
        ...
        balance_sheet = coordinator.accountant.generate_balance_sheet(df)
        income_statement = coordinator.accountant.generate_income_statement(df)
        ratios = coordinator.accountant.calculate_ratios(df)

        pdf_bytes = pdf_generator.generate_pdf_report(
            filename=db_session.filename,
            kpis=worker_kpis,
            narrative="Automated Executive Financial & Analytics Audit Briefing generated by AI Data Dashboard.",
            key_findings=[
                f"Total Orders processed: {worker_kpis.get('total_orders', 0):,}",
                f"Total Revenue generated: £{worker_kpis.get('total_revenue', 0.0):,.2f}",
                f"Balance Sheet Status: {'100% Balanced (Assets = Liabilities + Equity)' if balance_sheet.get('summary', {}).get('is_balanced') else 'Audit Attention Required'}",
                f"Current Ratio: {ratios.get('liquidity', {}).get('current_ratio', 'N/A')} ({ratios.get('liquidity', {}).get('status', 'Healthy')})",
            ],
        ...
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=CFO_Audit_Report_{db_session.filename}.pdf"}
        )
    except HTTPException:
        raise
    except Exception as e:
